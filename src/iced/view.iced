    ...
    for i in [0...courseList.length]
        ...
        if id.search('-') is -1
            ...
            k = $(
                    ...
                    '<a href="#"><i class="icon-book"></i> ' + name + '</a>' +
                    '<ul class="subfolder">' +
                    '<li><a target="content-frame" href="' + URL_CONST['new_course_info']  + id + '"><i class="icon-bullhorn"></i> 课程信息与公告</a></li>' +
                    '<li><a target="content-frame" href="' + URL_CONST['new_course_file']              + id + '"><i class="icon-download-alt"></i> 课程文件</a></li>' +
                    '<li><a target="content-frame" href="' + URL_CONST['new_course_homework']          + id + '"><i class="icon-pencil"></i> 课程作业</a></li>' +
                    '<li><a target="content-frame" href="' + URL_CONST['new_group_learning']          + id + '"><i class="icon-question-sign"></i> 群组学习</a></li>' +
                    '<li><a target="content-frame" href="' + URL_CONST['new_forum']           + id + '"><i class="icon-comments"></i> 课程讨论</a></li>' +
                    '<li><a target="_blank"        href="' + URL_CONST['new_course_page']       + id + '"><i class="icon-external-link"></i> 在新窗口中打开</a></li>' +
                    '</ul>' +
                    '</li>'
                )
        GUIlist.append(k)
...
gui_main_createNewLine = (data) ->
    ...
    if data.type is 'd' # DDL
        ...
        if data.courseId.search('-') is -1
            line += '<a class="handin-link" target="content-frame" href="' + URL_CONST['deadline_submit'] + '?id=' + data.id + '&course_id=' + data.courseId + '">提交链接</a> '
        else
            line += '<a class="handin-link" target="content-frame" href="' + URL_CONST['new_deadline_submit'] + data.courseId + '/' + data.id + '">提交链接</a> '
        ...
        if data.resultState
            if data.courseId.search('-') is -1
                ...
                line += '<a class="review-link" target="content-frame" href="' + URL_CONST['new_deadline_review'] + data.courseId + '/' + data.id + '">查看批阅</a>'
        ...
        if data.courseId.search('-') is -1
            ...
            line += '<a class="title" target="content-frame" data-args="read"'+
                'href="http://learn.cic.tsinghua.edu.cn/b/resource/downloadFileStream/'+data.id+
                 '" title="' + data.name + '"><span class="tag theme-magenta"><i class="icon-download-alt"></i></span> ' + data.name + '</a></td>'
        ...
        line += '<a class="course-name" target="content-frame" href="' + URL_CONST['notification'] + '?course_id=' + data.courseId + '">' + data.courseName + '</a>'
        line += '</div>'
    else if data.type is 't' # DISCUSS
        line += 'discuss '
        line += 'is-' + data.state + ' '
        line += '" data-args=' + id + '> '
        if data.courseId.search('-') isnt -1
            line += '<a class="title" target="content-frame" data-args="read" href="'+
                URL_CONST['new_discuss_detail'] + data.courseId + '/' + data.id + 
